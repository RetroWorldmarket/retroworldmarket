...
const editProduct = async (req, res, next) => {
  ...
  try {
    ...
    // Antes que nada tenemos que acceder al producto (idProduct). Esto se hace mediante
    // el PathParam (:idProduct).
    // Obtenemos los PathParams del objeto req.params y, mediante destructuring, nos quedamos con
    // el id del producto:
    const { idProduct } = req.params;

    // Vamos a comprobar que el producto exista en la BD. Si no existe, lanzamos un error:
